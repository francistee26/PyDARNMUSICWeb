    ...
    try:
        if request.POST:
            ...
            if request.POST.get("multiproc") != None:
                multiproc = True
            print("INN")
            if not mstid_index and not music_process:
                print("INN22")
                messages.add_message(request, messages.ERROR, 'User must either classify MSTIDs or run music analysis.')
                return render(request,"classify.html")
